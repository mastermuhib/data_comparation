                ...
                <form id="form_filter">
                    <div class="row" data-select2-id="4">
                        ...
                        <div class="col-md-2">
                            ...
                            <input type="hidden" name="triwulan" value="" id="triwulan">
                            <input type="hidden" name="year" value="" id="year">
                        </div>
                        <div class="col-md-2">
                            <label>disabilitas</label>
                            <select class="select2 form-control" onchange="DataTable()" id="disabilitas" name="disabilitas">
                                <option value="">Semua</option>
                                @foreach($data_disabilitas as $k)
...
function data_tabel(table) {
    ...
    var nantable    = $('#yourTable').DataTable({
        ...
        "ajax": {
            ...
            "dataType": "json",
            "type": "POST",
            "data": { _token: "{{csrf_token()}}",id_kec:id_kec,year:year,triwulan:triwulan,disabilitas:disabilitas,status:status }
        },
        "columns": column,
        "bDestroy": true
    });
...
$(function() {
    ...
    data_tabel('/rekapitulasi/data_disabilitas')
});
...
$("#form_calcuclate").validate({
    submitHandler: function(form) {
        ...
        $.ajax({ //line 28
            type: 'POST',
            url: '/rekapitulasi/disabilitas/calculate',
            dataType: 'json',
...
function Download(type){
    var serial = $("#form_filter").serialize();
    window.open("/download_rekapitulasi_disabilitas/"+type+"/"+serial, '_blank');
}
</script>
@endsection